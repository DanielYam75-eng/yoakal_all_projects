...
SCORE = "score"
METRIC = "metric"
STD_CLUSTERS = pd.DataFrame.from_dict(
    {
        "0101/05": "medium std",
        "0101/06": "high std",
        "0101/13": "medium std",
...
def get_scores(data: pd.DataFrame, metric: Callable, slicing: str = IND) -> pd.Series:
    ...
    """
    scores = data.groupby(slicing).agg(
        lambda g: metric(data.loc[g.index, ACT], data.loc[g.index, PRED])
    )[ACT]
...
def main(forecast, truth, output, alternative=None):
    ...
    data = pd.merge(
        load_data(forecast[0], PRED),
        load_data(truth[0], ACT),
        left_index=True,
        right_index=True,
        how="outer",
    ).fillna(0)
    ...
    if alternative is None:
        ...
        alternative = load_data(alternative[0], PRED)
    ...
    for metric in r2_score, root_mean_squared_error, log_rmse, comparison:
        ...
        frames += [
            get_scores(data[data.index.str.startswith("1500")], metric, SLC),
            get_scores(data[data.index.str.startswith("1600")], metric, SLC),
        ]

        data[SLC] = data.index.str.split("/").str[-1]
        frames.append(
            get_scores(data.groupby(SLC).filter(lambda g: len(g) > 1), metric, SLC)
        )
    ...
    results.to_csv(output[0], index=True)
