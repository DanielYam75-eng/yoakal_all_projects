...
def preprocess(table1, table2):
    ...
    table2 = table2.drop_duplicates(
        subset=["doc_id", "fund_year"], keep="first"
    ).set_index(["doc_id", "fund_year"])
    # start month and year
    table2["start_year"] = pd.concat(
        [
            pd.Series(
                table2.index.get_level_values("fund_year").astype(int),
                index=table2.index,
            ),
            pd.to_datetime(table2["doc_date"], format="%d.%m.%Y").dt.year,
        ],
        axis=1,
    ).max(axis=1)
    table2["N"] = (
        table2.index.get_level_values("fund_year")
        - pd.to_datetime(table2["doc_date"], format="%d.%m.%Y").dt.year
    )
    ...
    table2 = table2.join(
        pd.cut(
            np.log(table2.loc[table2["po_net_value"] > 0, "po_net_value"]), bins=10
...
class Trainer:
    def __init__(
        self, model, data, features_list=["po_type", "N"], labels_list=[0, 1, 2, 3, 4]
    ):
        self.data = data
        self.model = model
        self.X_train = None
    ...
    def train_and_evaluate(self, split_year):
        ...
        self.model = self.split_and_fit(
            features_table, labels_table, split_year=split_year
        )
        ...
        return dict(
            ...
            rmse_out_of_sample=rmse_out_of_sample,
            r2_out_of_sample=r2_out_of_sample,
        )

    def _create_features_and_labels_tables(self):
        labels_table = (
            self.data.loc[:, self.labels_list]
            .div(self.data["po_net_value"], axis=0)
            .fillna(0)
        )
...
class Wrapper:
    ...
    def _get_X(self, df, target_year):
        ...
        df["opening_year"] = pd.to_datetime(
            df["doc_date"], format="%d.%m.%Y"
        ).dt.year.astype(int)
    ...
    def _get_results(self, y_pred, net_values, diff_years):
        ...
        results = (
            pd.DataFrame(diff_years)
            .join(y_pred, how="left", on=["doc_id", "fund_year", "diffyear"])
            .fillna(0)
            .join(net_values, how="left", on=["doc_id", "fund_year"])
        )
    ...
    def predict(self, df, target_year):
        ...
        net_values = df[["doc_id", "fund_year", "po_net_value"]].set_index(
            ["doc_id", "fund_year"]
        )["po_net_value"]
...
def read(key1, key2):
    ...
    table2 = rf.read(key2)
    table2["po_net_value"] = (
        table2["po_net_value"].astype(str).str.replace(",", "").astype(float)
    )
...
def train():
    ...
    feature_list3 = [
        "po_type",
        "fingroup",
        "procurment_organization",
        "N",
        "po_net_value_category",
        "quarter",
    ]
    xgboost_model3 = XGBRegressor(
...
def infer():
    ...
    # Augmentation
    temp = table2[
        (table2["doc_date"] > pd.Timestamp(base_date))
        & (table2["doc_date"] < pd.Timestamp(f"{target_year}-01-01"))
    ]
    ...
    if not is_fine:
        (
            pd.DataFrame(
                wrapper.predict(augmented_table2, target_year), columns=["prediction"]
            )
