...
def read_pickle(key_name: str):
    ...
    # When to raise an error:
    key = info_file_string(
        key_name, key_row["Source"], key_row["Creation Date"], key_row["Template"]
    )
    obj = boto_client.get_object(Bucket=bucketname, Key=key)

    data = pickle.load(io.BytesIO(obj["Body"].read()))
    return data
