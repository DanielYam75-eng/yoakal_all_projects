...
def main(path, current_year, coin_type, bucket):
    ...
    if coin_type == 1:
        ...
        data.loc[
            (data["doc_type"].isin(["KR", "KG"])) & (data["law"] == 300), "type"
        ] = "vehicles"  # should be before cor
        data.loc[(data["fund_code"] == 1411) & (data["doc_type"] == "ZC"), "type"] = (
            "career_salary"
        )
        data.loc[(data["fund_code"] == 1409) & (data["doc_type"] == "ZC"), "type"] = (
            "drafted_salary"
        )
        data.loc[
            (data["fund_code"].isin([1401, 1402]))
            & (data["doc_type"].isin(["ZC", "ZW"])),
            "type",
        ] = "pensions"
        data.loc[
            (data["fund_code"].isin([1408])) & (data["doc_type"] == "ZC"), "type"
        ] = "idf_workers_salary"
        data.loc[
            (data["fund_code"].isin([1413, 1414, 1415]))
            & (data["doc_type"].isin(["ZC", "ZW"])),
            "type",
        ] = "dd_workers_salary"
        data.loc[(data["fund_code"] == 1412) & (data["doc_type"] == "ZC"), "type"] = (
            "pre_draft_salary"
        )
        data.loc[(data["fund_code"] == 1416) & (data["doc_type"] == "ZC"), "type"] = (
            "additional_drafted_service_salary"
        )
        data.loc[
            (data["doc_type"].isin(["KR", "KG"])) & (data["law"] == 302), "type"
        ] = "overseas_transportation"
        data.loc[
            (data["doc_type"].isin(["KR", "KG"])) & (data["law"] == 706), "type"
        ] = "tariffs"
        data.loc[
            (data["doc_type"].isin(["KR", "KG"])) & (data["law"] == 2900), "type"
        ] = "insurance"
        data.loc[
            (data["doc_type"].isin(["KR", "KG"])) & (data["law"] == 1316), "type"
        ] = "special_compensation"
        data.loc[(data["law"] == 9800), "type"] = "special_research"
        data.loc[(data["fund_code"].isin([1423])), "type"] = "hostages"
        data.loc[(data["fund_code"].isin([1425])), "type"] = "fiancees"
    ...
    if coin_type == 5:
        ...
        data.loc[
            (data["doc_type"].isin(["KR", "KG"]))
            & (data["expenditure_type"].isin([2010, 2030, 2050, 2015, 2035, 2045])),
            "type",
        ] = "travel-KRKG"
        data.loc[
            (data["doc_type"].isin(["KR", "KG"]))
            & (data["fund_code"].isin(range(1400, 1479))),
            "type",
        ] = "14-KRKG"
    ...
    frames = {
        ...
        .groupby(
            [
                frames[name].index.get_level_values(0),
                frames[name].index.get_level_values(1).year,
            ]
        )
        .cumsum()
        for name in frames
    ...
    for name, frame in frames.items():
        if not frame.empty and f"{current_year}-01-31" in frame.index.levels[1]:
            ...
            print(f"Frame {name} is empty. No data to save.", file=sys.stderr)
